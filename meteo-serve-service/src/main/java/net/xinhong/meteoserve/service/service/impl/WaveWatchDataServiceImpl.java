 ...
 */
@Service
public class WaveWatchDataServiceImpl implements WaveWatchDataService {
    private static final Log logger = LogFactory.getLog(WaveWatchDataServiceImpl.class);
    float delta = 0.5f;
    @Autowired
    private WaveWatchDataDao waveWatchDataDao;
    ...
    public JSONObject getPointData(String lat, String lng, String year, String month,
        ...
        WaveWatchDataServiceImpl.ResultTime restime = getResultTime(useDate);
        if (restime == null) {
        ...
        String strLat = String.format("%.2f",
                Math.round(Math.round((flat)*1000)/(delta*1000))*delta);
        String strLng = String.format("%.2f",
                Math.round(Math.round(flng*1000)/(delta*1000))*delta);
        JSONObject dataJSON = waveWatchDataDao.getPointData(strLat, strLng, ryear, rmonth, rday, rhour, VTI);
    ...
    @Override
    public JSONObject getImg(String year, String month, String day, String hour, String eles) {
        JSONObject resJSON = new JSONObject();
        DateTimeFormatter dateformat = DateTimeFormat.forPattern("yyyyMMddHH");
        DateTime useDate = DateTime.parse(year + month + day + hour, dateformat); //传入为北京时
        WaveWatchDataServiceImpl.ResultTime restime = getResultTime(useDate);
        ...
        JSONObject daoJSON = waveWatchDataDao.getImg(ryear, rmonth, rday, rhour, VTI, eles);
    ...
    public JSONObject getIsoLineData(String year, String month, String day, String hour, String s, String elem, String strArea) {
        ...
        String rday = resDate.toString("dd");
        String rhour = resDate.toString("HH");
        JSONObject daoJSON = waveWatchDataDao.getIsolineData(ryear, rmonth, rday, rhour, VTI, elem, strArea);
    ...
    public JSONObject getSpaceProfileData(String strLats, String strLngs, String year, String month, String day,
        ...
        for (int i = 0; i < profileLnglats.length; i++) {
            ...
            JSONObject tdaoJSON = waveWatchDataDao.getPointData(strLat, strLng, ryear, rmonth, rday, rhour, VTI);
            JSONObject daoJSON = new JSONObject();
            if (daoJSON != null) daoJSON.put(ResJsonConst.DATA, tdaoJSON);
    ...
    public JSONObject getTimeProfileData(String strLat, String strLng, String year, String month, String day,
        ...
        for (int i = 0; i < hourNum; i += 3) {
            ...
            JSONObject tdaoJSON = waveWatchDataDao.getPointData(resLat, resLng, ryear, rmonth, rday, rhour, VTI);
            JSONObject daoJSON = new JSONObject();
            if (tdaoJSON != null) {
                daoJSON.put(ResJsonConst.DATA, tdaoJSON);
            }
...
//            strArea = "EN"; //默认东北半球数据
        ...
        JSONObject daoJSON = waveWatchDataDao.getAreaData(ryear, rmonth, rday, rhour, VTI, elem);
        if (this.isEmptyJSON(daoJSON)){ //找前一个起报时间
            ...
            daoJSON = waveWatchDataDao.getAreaData(ryear, rmonth, rday, rhour, VTI, elem);
        }
    ...
    private WaveWatchDataServiceImpl.ResultTime getResultTime(DateTime useDate) {
        //3小时间隔
...
//        }
        ...
        WaveWatchDataServiceImpl.ResultTime restime = new WaveWatchDataServiceImpl.ResultTime();
