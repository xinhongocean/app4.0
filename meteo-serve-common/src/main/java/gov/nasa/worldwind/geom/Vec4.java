...
/**
 * @author dcollins
 * @version $Id: Vec4.java 1 2011-07-16 23:22:47Z dcollins $
 */
public class Vec4
{
    ...
    public static Vec4 fromDoubleArray(double[] array, int offset, int length)
    {
        ...
        if (offset < 0)
        {
            throw new IllegalArgumentException("generic.OffsetIsInvalid");
        }

        if (length < 1)
        {
            throw new IllegalArgumentException("generic.LengthIsInvalid");
        }

        if (array.length < offset + length)
        {
            throw new IllegalArgumentException("generic.ArrayInvalidLength");
        }
    ...
    {
        // For a discussion of homogeneous(齐次的，均匀的) coordinates, see "Mathematics for 3D Game Programming and Computer Graphics,
    ...
    public static Vec4 fromLine3(Vec4 origin, double t, Vec4 direction)
    {
        ...
        {
            throw new IllegalArgumentException("nullValue.Vec4IsNull");
        }
    ...
    public final Vec4 divide3(double value)
    {
        if (value == 0)
        {
            throw new IllegalArgumentException("generic.ArgumentOutOfRange");
        }
    ...
    public static Angle axisAngle(Vec4 v1, Vec4 v2, Vec4[] result)
    {
        ...
        Vec4 u1 = v1.normalize3();//求单位向量 db_x 2014-7-9
        Vec4 u0 = v2.normalize3();
        Angle angle = Angle.fromRadians(Math.acos(u0.x * u1.x + u0.y * u1.y + u0.z * u1.z));

        // Compute rotation axis 
        //向量外积 a×b = (a2*b3 - a3*b2)*e1 + (a3*b1 - a1*b3)*e2 + (a1*b2 - a2*b1)*e3 2014-7-9 db_x
    ...
    public static Vec4 computeAveragePoint(Iterable<? extends Vec4> points)
    {
        if (points == null)
        {
            throw new IllegalArgumentException("nullValue.PointListIsNull");
        }
    ...
    public static Vec4[] computeExtrema(Vec4 points[])
    {
        if (points == null)
        {
            throw new IllegalArgumentException("nullValue.PointsArrayIsNull");
        }
